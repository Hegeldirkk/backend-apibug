import { Injectable, BadRequestException, UnauthorizedException, Body } from '@nestjs/common';
...
export class AuthService {
  ...
  async register(@Body() dto: RegisterDto, role: UserRole) {
    ...
    if (existing) {
      throw new BadRequestException({ success: false, message:'Email déjà utilisé'});
    }
  ...
  async login(dto: LoginDto) {
    ...
    if (!user || !(await bcrypt.compare(dto.password, user.password))) {
      throw new UnauthorizedException({success: false, message:'Identifiants invalides'});
    }
   ...
   async verifyAccount(token: string) {
    try {
      ...
      if (!user) {
        ...
        throw new BadRequestException({success: false, message:'Utilisateur non trouvé'});
      }
  ...
  async changePassword(userId: string, dto: ChangePasswordDto) {
    ...
    if (!passwordValid) throw new BadRequestException('Ancien mot de passe incorrect');
